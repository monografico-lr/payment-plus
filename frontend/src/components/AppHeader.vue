        ...
        <nav class="user-controls">
          <li class="navButton">
            <a
              ...
              title="Caja Chica"
              @click.prevent="openPettyCash('retire')"
            >
...
export default {
  ...
  methods: {
    ...
    getSaldo() {
      axios.get('petty_cash/get_balance').then(res => {
        this.store.setPettyCashBalance(res.data);
      });
    },

    getDayIncome() {
      axios.get('report/get_day_income').then(res => {
